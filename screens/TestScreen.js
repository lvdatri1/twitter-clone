...
import { ScrollView, FlatList, Platform, View } from "react-native";
...
export default class TestScreen extends Component {
    ...
    _renderItem = ({ item }) => < NBCard id = { item.id }
    data = { item }
    />;
    loadingPeople() {
    ...
    render() {
        const videoContent = Platform.OS === 'ios' ? < Video
        controls = { true }
        paused
        source = {
            {
                uri: "https://d3a519wjc57hdm.cloudfront.net/2018/12/08/8715343-1544235784.mp4"
            }
        }
        style = {
            {
        ...
        return ( <
            ...
            /Header> <
            View style = {
                { zIndex: 1, flex: 1, flexDirection: 'row', flexWrap: 'wrap' } } >
            <
            View style = {
                { width: 150, height: 150, backgroundColor: 'powderblue', flex: 0 } }
            /> <
            View style = {
                { width: 150, height: 150, backgroundColor: 'skyblue', flex: 0 } }
            /> <
            View style = {
                { width: 150, height: 150, backgroundColor: 'steelblue', flex: 0 } }
            /> <
            View style = {
                { width: 150, height: 150, backgroundColor: 'skyblue', flex: 0 } }
            /> <
